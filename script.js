            ...
            card.innerHTML = `
                ...
                <div class="card-info">
                    ...
                    <div class="video-stats">
                        ...
                        <span class="like-btn" onclick="event.stopPropagation(); handleCardLike('${video.id}')">
...
window.handleCardLike = (videoId) => {
    if (!currentUser) {
        showToast('سجل دخولك للإعجاب 💖', 'warning');
        showAuthModal('login');
        return;
    }
    toggleCardLike(videoId);
};
...
async function toggleCardLike(videoId) {
...
window.addComment = async () => {
    if (!currentUser) {
        showToast('سجل دخولك للتعليق 💬', 'warning');
        showAuthModal('login');
        return;
    }
